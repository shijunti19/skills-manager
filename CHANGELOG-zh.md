...
### 用户可见更新
- **启动任务还在跑时安装技能不再报「skills repository is busy」** —— 冷启动时后台 reindex 会持有仓库锁 40–100 秒，而安装命令只等 20 秒就放弃；确认安装失败时克隆的预览目录还会被删除，用户只能从头重新克隆。现在四个安装入口（本地目录、git URL、skills.sh、预览确认）都会最多等 150 秒；若仍报「busy」错误，错误信息会注明当前持锁者和已持有时长（如 `held by "reindex sync metadata" for 42s, pid 1234`），不再让人摸不着头脑。
- **启动时的技能更新检查不再因 reindex 未完成而整批失败** —— 启动时后台 reindex 会持有仓库锁数秒，而延迟 3s 触发的「检查全部技能更新」恰好在此时发起。由于逐技能取锁是非阻塞的，每个技能（如 224 个）都立即以「skills repository is busy」失败，整轮检查被静默放弃。现在批量检查会在逐技能循环前先等一次（最多 20s）让锁释放，前端对「busy」错误也会按 5s/15s/30s 退避重试。热重载时也不再出现 `[TAURI] Couldn't find callback id` 残留回调警告，因为 effect 现在用一个取消标志贯穿每个 await 边界。
- **智能标签编辑器的「绑定技能」现在点保存就会生效** — 之前在「绑定技能」输入框里粘贴技能名后直接点保存，会静默地什么都不绑定，因为只有「应用匹配」按钮才会把名字写进待绑定集合。现在保存时会自动把输入框里匹配到的技能名并入绑定集，未匹配到的会弹提示而不是被悄悄丢弃。

### 开发者与治理更新
- `RepoLock::acquire_install` 最多等待 `INSTALL_WAIT`（150 秒）—— 安装比普通前台操作的 20 秒 `FOREGROUND_WAIT` 预算更大，因为启动 reindex 可持锁 40–100 秒，且安装命令返回时会删除克隆的临时目录，调用方重试无法复用克隆。所有安装入口均已换用（桌面端：本地/git/skills.sh/预览确认；CLI：本地/git/skills.sh）。两条取锁路径的「busy」错误现在都会通过 `holder_note()` 注明持锁操作、已持有时长和 pid——它读取无锁的戳记副本 `.skills-manager.lock.info`（Windows 下锁文件被持有时本身不可读，os error 33），副本缺失或解析失败时优雅降级为不加注。
