...
fn ensure_gitignore(skills_dir: &Path) -> Result<()> {
    ...
    // Skip the rewrite when no required line is missing: touching the file's
    // mtime on every sync would make `git status` see a dirty worktree right
    // after a clean sync, forcing a spurious empty commit. (Note: this mtime
    // short-circuit is independent of the gitignore entry for last_reindex.json
    // — that entry stops the reindex cache from entering commits at all; this
    // one stops .gitignore itself from churning the worktree.)
