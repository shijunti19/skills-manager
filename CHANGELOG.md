...
### Developer & Governance
- `RepoLock::acquire_install` waits up to `INSTALL_WAIT` (150s) — installs get a bigger budget than the 20s `FOREGROUND_WAIT` because the startup reindex can hold the lock for 40–100s and install commands delete their clone temp directory on return, so a caller-side retry cannot reuse the clone. Used by all install entry points (desktop app: local/git/skills.sh/preview-confirm; CLI: local/git/skills.sh). "busy" errors from both acquire paths now name the current lock holder, held-for and pid via `holder_note()`, which reads an unlocked sidecar copy of the stamp (`.skills-manager.lock.info`) — the lock file itself cannot be read while locked on Windows (os error 33) — and degrades to no note when the sidecar is missing or unparsable.
